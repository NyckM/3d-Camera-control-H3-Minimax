...
export function createCameraEditor({read, write, duration, interpolation, setInterpolation, setDuration, linkedImage, elevationRange, switches, frameMode, setFrameMode, promptDetail, runtimeTask, motionFraction, loopClosure, setLoopClosure, experimentMode, setExperimentMode}) {
  ...
  cameraView.innerHTML='<div style="font-size:11px;color:#b7c9db;margin-bottom:6px">PT: Visão da câmera · manequim 3D · FOV ilustrativo 40° EN: Camera view · 3D mannequin · illustrative FOV 40°</div><canvas aria-label="Camera view" style="display:block;width:100%;height:230px"></canvas>';
  ...
  function update(rebuild=true) {
    ...
    for(const el of root.querySelectorAll('[data-field]')){const v=path[selected][el.dataset.field];if(document.activeElement!==el)el.value=Number(v.toFixed(3));el.disabled=selected===0;}
  ...
  function draw(p){
    ...
    if(!cameraView.hidden)drawCameraView(cameraView.querySelector('canvas'),p,aspect);
  ...
  function modify(field,value){if(selected===0||!Number.isFinite(value))return;const e=elevLimit(),limits={azimuth:[-11520,11520],elevation:[-e,e],distance:[.1,4]};path[selected][field]=snap[field](clamp(value,...limits[field]));if(!playing)playhead=path[selected].time*activeFraction();save(false);}
  ...
  return {element:root,sync(){modeSelect.value=frameMode?.()||'Freeze Frame';sync();refreshReference();},destroy(){disposed=true;stop();media.destroy();clearInterval(poll);observer.disconnect();}};
}
