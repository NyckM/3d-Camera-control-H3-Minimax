...
export function createCameraEditor({read, write, duration, interpolation, setInterpolation, setDuration, linkedImage, elevationRange, switches, frameMode, setFrameMode, promptDetail, runtimeTask, motionFraction, loopClosure, setLoopClosure, experimentMode, setExperimentMode, depthWarp, setDepthWarp, depthOffset}) {
  ...
  <div><div class="heading"><span>Height</span><input class="number" aria-label="Camera height" type="number" data-field="height" min="-3" max="3" step="0.05"></div><input class="range" aria-label="Height slider" data-field="height" type="range" min="-3" max="3" step="0.01"></div>
  <div><div class="heading"><span>Lateral</span><input class="number" aria-label="Camera lateral" type="number" data-field="lateral" min="-3" max="3" step="0.05"></div><input class="range" aria-label="Lateral slider" data-field="lateral" type="range" min="-3" max="3" step="0.01"></div></div>
  ...
  cards[4].querySelector('.heading span').textContent='Lateral';
  ...
  const snap={azimuth:v=>Math.round(v),elevation:v=>Math.round(v),distance:v=>Math.round(v*100)/100,height:v=>Math.round(v*100)/100,lateral:v=>Math.round(v*100)/100};
  function modify(field,value){if(selected===0||!Number.isFinite(value))return;const e=elevLimit(),limits={azimuth:[-11520,11520],elevation:[-e,e],distance:[.1,4],height:[-3,3],lateral:[-3,3]};path[selected][field]=snap[field](clamp(value,...limits[field]));if(!playing)playhead=path[selected].time*activeFraction();save(false);}
  ...
  function sync(){const fraction=activeFraction();if(fraction!==previousFraction){stop();playhead=clamp(playhead/previousFraction*fraction,0,1);previousFraction=fraction;}const raw=read();if(raw!==lastRaw){try{const data=JSON.parse(raw);if(!Array.isArray(data)||data.length<2||data.length>24)throw Error();for(let i=0;i<data.length;i++){const p=data[i];if(p.height==null)p.height=0;if(p.lateral==null)p.lateral=0;if(!['time','azimuth','elevation','distance','height','lateral'].every(k=>typeof p[k]==='number'&&Number.isFinite(p[k]))||p.time<0||p.time>1||p.distance<.1||p.distance>4||Math.abs(p.elevation)>89||Math.abs(p.height)>3||Math.abs(p.lateral)>3||(i&&p.time<=data[i-1].time))throw Error();}const first=data[0];if(first.time!==0||first.azimuth!==0||first.elevation!==0||first.distance!==1||first.height!==0||first.lateral!==0)throw Error();path=data;lastRaw=raw;toolError='';selected=Math.min(selected,path.length-1);playhead=path[selected].time*activeFraction();}catch{$('.error').textContent='Trajetória inválida. Corrija o JSON ou use Reset. O primeiro ponto deve ser 0, 0°, 0°, distância 1.';return;}}update();}
