...
assert.equal(JSON.parse(await page.locator('#trajectory').inputValue())[1].azimuth,changed[1].azimuth+1);
await page.locator('[data-action=expand]').click();
await page.locator('[data-action=play]').click();
...
await page.screenshot({path:path.join(require('os').tmpdir(),'h3-v25-tested.png'),fullPage:true});
