...
await page.locator('[data-action=play]').click();
const original=await page.locator('#trajectory').inputValue();
const interpolation=await page.locator('#interpolation').inputValue();
await page.locator('[aria-label="Camera preset"]').selectOption('C-04');
assert.equal(await page.locator('#trajectory').inputValue(),original,'select must not mutate');
await page.locator('[data-action=preset-apply]').click();
let preset=JSON.parse(await page.locator('#trajectory').inputValue());assert.equal(preset.at(-1).azimuth,360);
assert.equal(await page.locator('#interpolation').inputValue(),'linear');
await page.locator('[data-action=preset-undo]').click();assert.equal(await page.locator('#trajectory').inputValue(),original);
assert.equal(await page.locator('#interpolation').inputValue(),interpolation);
await page.locator('[aria-label="Camera preset"]').selectOption('C-09');
assert.equal(await page.locator('[data-action=preset-apply]').isDisabled(),true);
await page.locator('[aria-label="Camera preset"]').selectOption('C-01b');await page.locator('[data-action=preset-apply]').click();
preset=JSON.parse(await page.locator('#trajectory').inputValue());assert.ok(Math.abs(preset[1].time*5.125-2)<1e-8);
assert.equal(preset[1].distance,1);assert.equal(preset[2].distance,.25);
console.log('PASS: preset selection non-destructive; apply, interpolation, undo, unsupported entry and normalized hold timing.');
await page.screenshot({path:path.join(require('os').tmpdir(),'h3-v28-tested.png'),fullPage:true});
